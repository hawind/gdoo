...
var reim = {
	...
	showhistorys: function (d, pad, lex) {
		...
		s += '<td align="left" width="100%"><div title="' + na + '" class="name">' + na + '' + s1 + '</div><div class="huicont">' + d.cont + '</div></td>';
	...
	openchat: function (type, reid, na, fac) {
		...
		var s = '<div style="background:#f0f3f4">';
		s += '<div id="viewtitle_' + num + '" style="height:50px;overflow:hidden;background-color:#f1f1f1;border-bottom:#dddddd solid 1px;">';
		s += '</div>';
		var hei = 206;
		s += '<div resizeh="' + hei + '" id="viewcontent_' + num + '" style="height:' + (viewheight - hei) + 'px;overflow:hidden;position:relative;"><div style="margin-top:50px" align="center"><img src="/assets/chat/images/mloading.gif"></div></div>';
		s += '<div style="background:#fff">';
		s += '<div class="toolsliao" id="toolsliao_' + num + '">';
		s += '	<span title="表情" tools="emts" class="cursor"><i class="fa fa-smile-o"></i></span>';
		s += '	<span title="文件/图片" tools="file" class="cursor"><i class="fa fa-folder"></i></span>';
		if (nwjsgui) {
			s += '	<span title="粘贴图片" tools="paste" class="cursor"><i class="fa fa-paste"></i></span>';
			s += '	<span title="截屏" tools="crop" class="cursor"><i class="fa fa-cut"></i></span>';
		}
		s += '</div>';

		s += '<div style="height:80px;overflow:hidden;"><div style="height:70px;margin:5px"><textarea onpaste="im.readclip(\'' + num + '\',event)"  class="content" id="input_content_' + num + '"></textarea></div></div>';

		s += '<div style="height:40px;overflow:hidden;"><div align="right" style="padding:9px"><input id="chatclosebtn_' + num + '" class="webbtn" type="button" value="关闭(C)">&nbsp;<input class="webbtn" id="chatsendbtn_' + num + '" type="button" value="发送(S)"></div></div>';
		s += '</div>';
		s += '</div>';
	...
	showagent: function (sbo) {
		...
		s += '<div id="agenhview" resizeh="0" style="height:' + viewheight + 'px;overflow:hidden;position:relative; background:#f0f3f4" align="center"><div style="width:80%;padding:20px" align="left">';
		agenharr = {};
		for (ty in agedt) {
			a = agedt[ty];
			len = a.length;
...
function chatcreate(cans) {
	...
	this.showtitle = function () {
		...
		s += '	<div><span style="font-size:16px;">' + this.name + '</span>';
		...
		if (this.type == 'group') {
			if (!od.deptid || od.deptid == '0') {
				s += '<td width="30" title="邀请" id="yaoqingchat_' + this.num + '" class="chattitbtn" nowrap><i class="fa fa-plus"></i></td>';
				s += '<td width="30" title="退出会话" id="tuichuchat_' + this.num + '" class="chattitbtn" nowrap><i class="fa fa-sign-out"></i></td>';
			}
			s += '<td width="30" title="会话里的人员" id="tuiuserlist_' + this.num + '" class="chattitbtn" nowrap><i class="fa fa-group"></i></td>';
		}
		s += '</tr></table>';
		o.html(s);
	...
	this.loaddata = function (o1, iref) {
		...
		if (o1) $(o1).html('<img src="/assets/chat/images/loadings.gif" height="14" width="15" align="absmiddle"> 加载中...');
		this.boolload = true;
	...
	this.loaddatashow = function (ret, isbf, isls) {
		...
		if (len > 0 && !isls) {
			...
			if (ret.wdtotal == 0) {
				s += '以上是历史记录';
				if (len >= 5) {
					this.showobj.prepend('<div id="loadmored_' + this.num + '" class="showblanks" ><a href="javascript:;" onclick="' + this.objstr + '.loadmoreda(this)"><i class="fa fa-clock-o"></i> 查看更多消息</a></div>');
					this.isshangla = true;
				}
			} else {
	...
	this.yaoqings = function (sid) {
		js.msg('wait', '邀请中...');
		reim.ajax(reim.getapiurl('/chat/chat/inviteUser'), {val: sid, gid: this.gid}, function (da) {
			js.msg();
			if (da.success) {
				js.msgok('邀请成功');
	...
	this.getreceinfor = function () {
		reim.ajax(reim.getapiurl('/chat/chat/getReceiver'), {type: this.type, gid: this.gid}, function (ret) {
	...
	this.showhuilist = function () {
		var s = '<div id="showuserlist" style="height:250px;overflow:auto;padding:5px 10px"><div align="center" style="padding:10px;"><img src="/assets/chat/images/mloading.gif" align="absmiddle">&nbsp;加载人员...</div></div>';
		js.tanbody('syscogshow', '会话上人员(' + this.usershu + ')', 420, 100, { html: s });
		if (!this.userlistarr) {
			reim.ajax(reim.getapiurl('/chat/chat/getGroupUser'), { type: this.type, gid: this.gid}, function (ret) {
	...
	this.exitgroups = function () {
		js.msg('wait', '退出中...');
		reim.ajax(reim.getapiurl('/chat/chat/exitGroup'), { gid: this.gid }, function (da) {
	...
	this.receivedata = function (d) {
		...
		reim.ajax(reim.getapiurl('/chat/chat/getRecord'), {type: this.type, gid: this.gid, minid: 0, lastdt: this.lastdt, page: this.page }, function (ret) {
	...
	this.sendconts = function (conss, nuid, optdt, fid) {
		this.sendbool = true;
		var d = {event: 'message', cont: conss, gid: this.gid, type: this.type, nuid: nuid, optdt: optdt, fileid: fid};
		reim.ajax(reim.getapiurl('/chat/chat/sendMessage'), d, function (ret) {
			me.sendsuccess(ret.data, nuid);
		}, 'post', function () {
			me.senderror(nuid);
		});
		var s1 = '';
		if (this.type == 'group') s1 = adminname + ':';
	...
	this.senderror = function (nuid) {
		get(nuid).src = '/assets/chat/images/error.png';
		get(nuid).title = '发送失败';
		this.sendbool = false;
	};
	...
	this.getemtsbq = function (wj, oi1, oi2, fzd, dx) {
		...
		for (i = oi1; i <= oi2; i++) {
			oi++;
			s += '<td width="' + j1 + '%" title="' + strformat.emotsarr[i + 1] + '" align="center"><img onclick="im.backemts(\'' + strformat.emotsarr[i + 1] + '\')" src="/assets/chat/images/im/emots/' + wj + '/' + i + '.gif" width="' + dx + '" height="' + dx + '"></td>';
			if (oi % fzd == 0) s += '</tr><tr>';
		}
		s += '</tr></table>';
		return s;
	...
	this.sendfileok = function (f, str) {
		...
		this.sendconts(conss, tm.nuid, tm.optdt, a.id);
	};
	...
	this.rightqipaoclick = function (d) {
		...
		if (lx == 1) {
			...
			if (!isNaN(ids)) {
				reim.ajax(reim.getapiurl('/chat/chat/clearRecord'), {type: this.type, gid: this.gid, ids: [ids]});
			}
		}
		if (lx == 3) {
		...
		if (lx == 2 && !isNaN(ids)) {
			...
			reim.ajax(reim.getapiurl('/chat/chat/recallMessage'), { type: this.type, gid: this.gid, id: ids }, function (ret) {
				o1dd.html(js.getmsg(ret.data.content, 'green'));
			}, 'get', function () {
